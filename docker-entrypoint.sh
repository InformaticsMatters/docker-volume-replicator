...
#   USE_FIRST_OF_MONTH_FOR_RCLONE Set to 'yes' to use a "First-Of-Month" destination sub-directory on the 1st of the month (and "Daily" on other days)
...
if [ "$USE_RCLONE" == "yes" ]; then
  ...
  # If so, "First-Of-Month" is used on the 1st day of the month
  # and "Daily" is used on all other days.
  # Either replaces any DOW sub-directory.
